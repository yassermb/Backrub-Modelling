...
def extract_structures( struct_db, rename_function = None ):
    args = [
        os.path.expanduser( '~/myStage/Softwares/Rosetta/rosetta_src_3.10_bundle/rosetta_src_2018.33.60351_bundle/main/source/bin//score_jd2.linuxgccrelease' ),
    ...
    if rename_function != None:
        ...
        for path in [ os.path.join( working_directory, x ) for x in os.listdir( working_directory ) ]:
            ...
            model_number = path[path.rfind('/')-2:path.rfind('/')]            
            
            m = re.match( '(\d+)_0001.pdb', os.path.basename(path) )
            if m:
                dest_path = os.path.join( working_directory, rename_function( int(m.group(1)), model_number ) )
                shutil.move( path, dest_path )

    return return_code

def flex_ddG_rename(struct_id, model_number):
    steps = [
        'backrub',
        'wt',
        'mut',
    ...
    return '%s_%s_%05d.pdb' % ( steps[ (struct_id-1) % len(steps) ], model_number, (((struct_id-1) // len(steps)) + 1) * trajectory_stride )
